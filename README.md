...
- 分辨率按模型提供 `1K / 1.5K / 2K / 3K / 4K`，选“自动”画幅时分辨率自动忽略
...
2. 启动原生桌面客户端：
   ...
   ```
    ...
    勾选“记住 Base URL 和 API Key”后，两项会使用 Windows DPAPI 加密保存到程序目录的 `client-credentials.dat`，
...
凭据保存是可选的：不勾选“记住 Base URL 和 API Key”时，关闭程序不会留下凭据文件。
...
- Seedream 优先向支持精确像素的模型发送推荐 `宽x高`，避免只传 `1K/2K` 档位时由上游自行决定较低尺寸；如果中转站拒绝精确像素，会依次回退到档位和省略 `size`。
- 生成完成后显示的是图片文件的真实像素尺寸，而不是请求档位。Seedream 5.0 lite 按官方能力提供 `2K / 3K`，不提供 4K。
- GPT Image 2 / 2.5 使用 `1K / 2K / 4K` 档位与推荐标准像素表；质量保持默认 `auto` 时按档位映射为 `standard / hd / 4k`，例如 `16:9 + 2K → 2560x1440 + hd`，也可手动指定 `standard / hd / 4k`。
